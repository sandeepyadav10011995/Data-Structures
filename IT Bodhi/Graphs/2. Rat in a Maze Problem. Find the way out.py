"""
Problem -: Rat in a maze Problem. Find the way out
...
class Solution:
    ...
    def minDistBetweenSourceAndDestination(matrix, srcRow, srcCol, destRow, destCol):
        ...
        visitedMap[srcRow*CS + srcCol] = srcRow*CS + srcCol
    ...
    @staticmethod
    def generatePath(visitedMap, srcRow, srcCol, destRow, destCol, CS):
        row = destRow
        col = destCol
        pathList = [(destRow, destCol)]
        while row != srcRow and col != srcCol:
            parentElement = visitedMap.get(row*CS+col)
            row = parentElement // CS
            col = parentElement % CS
            pathList.append((row, col))
        pathList.append((srcRow, srcCol))
        return pathList[::-1]
