...
class Solution:
    def THBBT(self, N):
        ...
        return self.THBBT(N-1)*self.THBBT(N-1) + self.THBBT(N-1)*self.THBBT(N-2) + self.THBBT(N-2)*self.THBBT(N-1)

    """
    Branch = 3 but for every branch we have 2 sub-branches ==> 6
    Level = N
    TC: O(6^N)
    SC: O(N) --> Call Stack
    """
