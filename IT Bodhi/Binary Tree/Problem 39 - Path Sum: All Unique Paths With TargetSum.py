...
class Variant1Solution:
    ...
    def _pathSumUtil(self, node, remSum, pathNodes=None, pathsList=None) -> None:
        ...
        if self._isLeaf(node) and node.val == remSum:
            pathsList.apppend(pathNodes[:])
        else:
    ...
    def pathsWithTargetSum(self, root, targetSum) -> list[list[int]]:
        pathsList = []
        self._pathSumUtil(root, targetSum, pathNodes=[], pathsList=pathsList)
        return pathsList
...
class BrutForceSolution:
    ...
    def _totalPathsFromThisNode(self, node, targetSum) -> int:
        # Base Case
        if node is None:
            return 0

        totalPathsCompletedFromThisNode = 1 if targetSum == node.val else 0
        totalPathsCompletedFromThisNode += self._totalPathsFromThisNode(node.left, targetSum - node.val) + \
                                           self._totalPathsFromThisNode(node.right, targetSum - node.val)
