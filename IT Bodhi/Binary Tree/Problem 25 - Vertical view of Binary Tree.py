...
class Solution:
    @staticmethod
    def generateTopViewOfBinaryTree(root: TreeNode):
        verticalOrderMap = {}
        # Base Case
        if root is None:
    ...
    def setVerticalChildrenForEveryNodeOfBT(self, root: TreeNode):
        verticalOrderMap, minHD, maxHD = self.generateTopViewOfBinaryTree(root)
    ...
    def createVerticalViewLayerByLayerOfBT(self, root: TreeNode):
        verticalOrderMap, minHD, maxHD = self.generateTopViewOfBinaryTree(root)
        for i in range(minHD, maxHD+1):
...
def main12():
    ...
    bt.createBT()
    root1 = bt.root
    print(sol.createVerticalViewLayerByLayerOfBT(root1))
