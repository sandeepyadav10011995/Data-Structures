...
Solution 1: Inorder Traversal --> We will get two values which are swap them.
            ...
            8, 10, 12, 25, 18, 20, 21, 15, 28, 30, 36, 40, 55
                            |           |
            We will find out the two values where the value is less than the next value
            Important Points while finding out the two swapped nodes -:
                Exception 1: Found at node 18, therefore First node would be previous node
                Exception 2: Found at node 15, therefore Second node would be current node
            ...
            Exception 1: Found at node 12, therefore in consecutive swaps we will get only ONE EXCEPTION.
            Therefore;
                    First Node --> Previous Node
                    Second Node --> Current Node
