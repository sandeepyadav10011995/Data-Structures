...
class SumTree:
    def __init__(self, sum: int = 0, isSumT: Boolean = False):
        self.sum = sum
        self.isSumT = isSumT
...
class BTTopDownApproach:
    ...
    def isSumTree(self, root):
        ...
        if self.isLeaf(root):
            return True
        LV = root.left.val if self.isLeaf(root.left) else 2 * root.val
        RV = root.right.val if self.isLeaf(root.right) else 2 * root.val
